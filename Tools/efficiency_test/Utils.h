...
namespace muon_pog
{
  ...
  bool hasNoGenMatch(const muon_pog::Muon & muon,
		     const std::vector<muon_pog::GenParticle>  & gens,
		     float drCutVeto, float minGenPt = 5.)
  {
    ...
    for (auto & gen : gens)
      {
	if (fabs(gen.pdgId) == 13 && gen.pt > minGenPt)
	  {
  ...
  bool hasbGenMatch(const muon_pog::Muon & muon,
		    const std::vector<muon_pog::GenParticle>  & gens,
		    float drCutVeto, float minGenPt = 5.)
  {
    ...
    for (auto & gen : gens)
      {
	if (fabs(gen.pdgId) == 5 && gen.pt > minGenPt)
	  {
	    Float_t dr = deltaR(muTk.Eta(), muTk.Phi(), gen.eta, gen.phi);
	    if (dr < drCutVeto)
	      return true;
